...
<div class="report-container">
    ...
    <table>
        ...
        <tbody id="data">
            <?php
            $total = 0;
            $sql = "SELECT srt.note, srt.sold_price, srt.qty, srt.tax, srt.discount, invt.unit_price FROM sales_receipt_tbl srt INNER JOIN item_tbl it ON (srt.item_id=it.item_id) INNER JOIN inventory_tbl invt ON (it.item_id=invt.item_id) WHERE it.company_id='" . $_SESSION['company_id'] . "' AND srt.date_made='" . date("Y-m-d") . "'";
...
<script>
    $(document).ready(function () {
        // set page title
        $(document).prop('title', 'Daily Revenue Report | Ginum');
        ...
        $("#search").click(function () {
            if ($("#start_date").val() === "") {
                ...
                $.ajax({
                    url: "../../backend/data/get-revenue-report.php",
                    method: "POST",
                    data: {
