...
// Ensure company_id is available in session
session_start();
if (!isset($_SESSION['company_id'])) {
    $response['message'] = 'Company is not logged in.';
...
if (isset($_POST['promo_code'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        if ($current_date >= $promo['valid_from'] && $current_date <= $promo['valid_until']) {
            ...
            // If the promo code is "save8", check its usage count for the current company
            if (strtolower($promo_code) === "save8") {
                $usage_sql = "SELECT used_count FROM promo_code_usage_tbl WHERE company_id = ? AND promo_code = ?";
                $usage_stmt = $conn->prepare($usage_sql);

                if ($usage_stmt === false) {
                    $response['message'] = 'Database query failed for checking usage count: ' . $conn->error;
                    echo json_encode($response);
                    exit();
                }
                ...
                $used_count = $usage_data['used_count'] ?? 0;
                if ($used_count >= 12) {
                    $response['message'] = "Promo code '$promo_code' has reached its usage limit (12 months) for your company.";
                    $usage_stmt->close();
                    echo json_encode($response);
                    exit();
                }
