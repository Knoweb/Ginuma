...
export default function AgedPayables() {
  ...
  const fetchPurchaseOrders = async () => {
    try {
      ...
      console.log("Purchase Orders loaded for Aged Payables:", list);
  ...
  // Compute detail rows for all bills
  const detailData = useMemo(() => {
    return purchaseOrders.map((order) => {
      const balance = Number(order.balanceDue || 0);
      const dueDate = order.promiseDate || order.dueDate || order.issueDate;
  ...
  // Compute aggregated summary data by supplier
  const summaryData = useMemo(() => {
    ...
    detailData.forEach((row) => {
      ...
      supplierMap[row.supplier].age4 += row.age4;
      supplierMap[row.supplier].total += row.balance; // Accumulated outstanding balance
      supplierMap[row.supplier].totalPaid += row.amountPaid;
      supplierMap[row.supplier].totalInvoice += row.total;
    });
  ...
  const totals = useMemo(() => {
    ...
    purchaseOrders.forEach((po) => {
      ...
      const dueDate = po.promiseDate || po.dueDate || po.issueDate;
      const daysOverdue = getDaysOverdue(dueDate);
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen space-y-6 max-w-full overflow-x-hidden">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 pb-4 border-b border-gray-200">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Aged Payables</h1>
          <p className="text-sm text-gray-500 mt-1">
            Track and manage outstanding supplier liabilities across aging periods
          </p>
        </div>
      ...
      {isLoading ? (
        <div className="flex justify-center items-center py-24 bg-white rounded-2xl border shadow-sm">
          ...
          <span className="text-gray-600 text-lg font-medium">Loading Payables...</span>
        </div>
      ) : activeTab === "summary" ? (
